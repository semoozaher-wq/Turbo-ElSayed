...
# Arabic voice families supported by Microsoft Edge TTS.  Keep this list in the
# WebUI so Arabic users can find the most natural regional voices first without
# changing the provider's canonical voice IDs or the persisted configuration.
ARABIC_VOICE_PREFIXES = (
    "ar-eg-",
    "ar-sa-",
    "ar-ae-",
    "ar-jo-",
...
def _voice_language_priority(voice_name: str) -> tuple[int, str]:
    """Return a stable display priority without changing a provider voice ID.

    Arabic Edge voices are otherwise buried in the full Azure catalogue.  The
    ordering is deliberately presentation-only: the selected value remains the
    original provider string, so existing configs and task payloads stay
    backwards compatible.  Non-Arabic providers keep their normal alphabetical
    order after the Arabic voices.
    """
    normalized = str(voice_name or "").strip().lower()
    for priority, prefix in enumerate(ARABIC_VOICE_PREFIXES):
        if normalized.startswith(prefix):
            return priority, normalized
    return len(ARABIC_VOICE_PREFIXES), normalized


def _order_voice_options(voices, *, selected_voice=""):
    """Order voice choices predictably and keep the saved voice visible first."""
    unique_voices = list(dict.fromkeys(str(item) for item in (voices or []) if item))
    selected_voice = str(selected_voice or "")
    return sorted(
        unique_voices,
        key=lambda item: (
            0 if item == selected_voice else 1,
            *_voice_language_priority(item),
        ),
    )
...
def _task_dashboard_metrics(tasks):
    """Compute lightweight local metrics from the task summaries.

    These metrics intentionally use only the task storage already scanned by the
    WebUI.  They do not claim to be social-platform views or revenue, which
    would require authenticated provider analytics APIs and could be misleading
    when those integrations are not configured.
    """
    metrics = {"total": 0, "processing": 0, "complete": 0, "failed": 0, "videos": 0}
    for task in tasks or []:
        metrics["total"] += 1
        status = _task_state_filter_key(task)
        if status in metrics:
            metrics[status] += 1
        if task.get("video_file") and os.path.isfile(task["video_file"]):
            metrics["videos"] += 1
    return metrics
...
def _render_task_manager_panel(tasks=None):
    ...
    metrics = _task_dashboard_metrics(tasks)
    st.caption(tr("Task Dashboard"))
    metric_cols = st.columns(5)
    metric_cols[0].metric(tr("Total Tasks"), metrics["total"])
    metric_cols[1].metric(tr("Processing Tasks"), metrics["processing"])
    metric_cols[2].metric(tr("Completed Tasks"), metrics["complete"])
    metric_cols[3].metric(tr("Failed Tasks"), metrics["failed"])
    metric_cols[4].metric(tr("Generated Videos"), metrics["videos"])
...
def _render_audio_settings(panel, params):
    ...
    with panel:
        with st.container(border=True):
            ...
            # Keep the configured voice at the top, then surface Arabic regional
            # voices before the rest of the catalogue.  This is especially useful
            # for the large Azure list and does not alter the actual voice value.
            filtered_voices = _order_voice_options(
                filtered_voices,
                selected_voice=saved_voice_name,
            )
