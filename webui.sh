...
# The Python launcher owns dependency checks, port selection, and Web UI
# startup. Keep this script as a compatibility wrapper for existing commands.
if [ "${TURBO_USE_LEGACY_WEBUI_SH:-0}" != "1" ]; then
  exec "${PYTHON:-python3}" "$CURRENT_DIR/start_app.py" --mode webui "$@"
fi
